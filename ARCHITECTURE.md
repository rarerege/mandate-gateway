...
**Verification (`app/verification/`) is the actual security boundary, and it is plain code, not an LLM call.** Five independent checks: Ed25519 signature over a canonical byte string, expiry, single-use nonce (replay), a known trust root, and — added after an internal audit of this exact system found the gap — that the mandate's *claimed* `agent_platform` actually matches the platform its signing key is registered to. That last check exists because `agent_platform` used to be neither signed nor cross-checked: a mandate could claim any platform it liked, which would have quietly undermined two other things this system does (a trusted-platform step-up exemption, and the per-platform coordinated-burst detector in `app/reputation/store.py` — a ring could have relabelled itself across fake platforms to dodge exactly the check meant to catch it). It's fixed two ways at once: the platform is now part of the signed bytes (`NormalizedMandate.canonical_signing_bytes`), so it can't be relabelled in flight without breaking the signature, and it's cross-checked against the trust registry, so an agent can't just lie about its own platform using its own otherwise-valid key. `tests/test_verifier.py` has a dedicated test for each of those two distinct attacks. None of this depends on a language model's judgment on a given day, and none of it can be talked around by adversarial text hidden inside an order's `items_summary` (a live prompt-injection surface if it ever reached an LLM before this check runs — see §6 for where an LLM *is* still exposed to attacker-controlled input, and why that's a smaller, bounded risk rather than a solved one).
...
- **The rationale-writing LLM call is not fully injection-proof — this is a deliberately scoped, bounded risk, not a solved one.** `app/reasoning/reasoner.py::_llm_rationale` interpolates attacker-supplied strings (`agent_id`, `category`) into the prompt it sends Claude to write the merchant-facing explanation sentence. The decision itself cannot be changed this way — `_llm_rationale` runs strictly after `decide_action` has already fixed the outcome, and its return value is only ever a display string — but a crafted `agent_id` could in principle still try to manipulate what that *sentence* says. Worst case is a misleading or manipulated explanation, never a flipped approve/reject. Worth saying plainly rather than claiming zero injection surface anywhere in the system.
- **A genuine gap was found and fixed mid-build, not swept under the rug: `agent_platform` used to be neither signed nor verified against the trust registry.** A mandate could claim to be from any platform it liked, with no cryptographic consequence — which would have quietly undermined the trusted-platform step-up exemption and let a ring dodge the coordinated-burst detector above by relabelling itself across fake platform names. Fixed in `app/verification/verifier.py` and `app/schemas.py::canonical_signing_bytes` (see §3) with two dedicated regression tests (`tests/test_verifier.py`) proving each of the two distinct attacks it closes. Flagged here deliberately: a security-focused project that never found and fixed anything in its own threat model would be a worse signal than one that did.
